#!/usr/bin/env bash
# Acceptance for seeds/marketplace/strict-false-conflict/ (Rust SDK)
# Axis: MK-STR-FALSE-CONFLICT — assert 'marketplace add' registers 0 skill(s).
...
"${A2C[@]}" marketplace add "file://$TMPDIR/bare.git" --name "strict-false-conflict" --trust --json >> "$LOG" 2>&1 || fail "marketplace add failed"
grep -q '"skills": 0' "$LOG" || fail "expected skills=0, got: $(grep '\"skills\"' "$LOG" || true)"
echo "PASS: marketplace seed ${SEED_NAME} (skills=0)"
