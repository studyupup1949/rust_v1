#!/usr/bin/env bash
# Acceptance for seeds/marketplace/strict-false-clean/ (Rust SDK)
# Axis: MK-STR-FALSE-CLEAN — assert 'marketplace add' registers 2 skill(s).
...
"${A2C[@]}" marketplace add "file://$TMPDIR/bare.git" --name "strict-false-clean" --trust --json >> "$LOG" 2>&1 || fail "marketplace add failed"
grep -q '"skills": 2' "$LOG" || fail "expected skills=2, got: $(grep '\"skills\"' "$LOG" || true)"
echo "PASS: marketplace seed ${SEED_NAME} (skills=2)"
