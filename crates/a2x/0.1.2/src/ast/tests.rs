...
fn test_process_operator() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let o = process_operator(&op_pair, None)?;
    assert_eq!(
        o.ns,
        vec!["main".to_string(), "foo".to_string(), "bar".to_string()]
    );
    assert_eq!(o.operator, "+/".to_string());
    Ok(())
}
...
fn test_cond_display_literal_string() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cond = process_condition(pair, &vec![], None, &Rc::new(Context::default()))?;
...
fn test_cond_pratt_simple() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let parsed = process_condition(pair, &vec![], None, &Rc::new(Context::default()))?;
    assert!(parsed.ns.is_empty());
    let parsed_cond_expr = parsed.cond_expr;
    match parsed_cond_expr {
        CondExpression::Infix(a, o, b) => {
            if let CondExpression::Lit(Constant::Integer(three)) = *a {
                assert_eq!(three, "3".to_string());
            } else {
                panic!("expected a literal integer (3) as first arg");
            }
            if let CondExpression::Lit(Constant::Integer(three)) = *b {
                assert_eq!(three, "3".to_string());
            } else {
                panic!("expected a literal integer (3) as second arg");
            }
            assert_eq!(o.operator, "+".to_string());
        }
        _ => {
...
fn test_cond_pratt_single_right_assoc() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let parsed = process_condition(pair, &vec![], None, &Rc::new(Context::default()))?;
    assert!(parsed.ns.is_empty());
    let cond_expr = parsed.cond_expr;
    if let CondExpression::Infix(literal, op, infix) = cond_expr {
        assert_eq!("&", op.operator);
        if let CondExpression::Lit(x) = *literal {
            assert_eq!("1", x.to_string());
        } else {
            panic!("expected literal 1 as first value");
        }
        // second level
        if let CondExpression::Infix(literal, op, infix) = *infix {
            assert_eq!("&", op.operator);
            if let CondExpression::Lit(x) = *literal {
                assert_eq!("2", x.to_string());
            } else {
                panic!("expected literal 2 as second value");
            }
            // third level
            if let CondExpression::Infix(literal, op, last_lit) = *infix {
                assert_eq!("&", op.operator);
                if let CondExpression::Lit(x) = *literal {
                    assert_eq!("3", x.to_string());
                } else {
                    panic!("expected literal 3 as third value");
                }
                if let CondExpression::Lit(x) = *last_lit {
                    assert_eq!("4", x.to_string());
                } else {
                    panic!("expected literal 4 as fourth value");
                }
            } else {
                panic!("unexpected condition expression structure");
            }
        } else {
            panic!("structure does not match");
        }
    }
    Ok(())
}
