...
pub enum ParseError {
    ...
    AstConvertError,
    #[error("Conversion into AST error: {}", _0)]
    AstMsgConvertError(String),
...
impl SrcError {
    ...
    pub fn err_opt(msg: &str, label: &str, src_loc: Option<&SrcLoc>) -> ParseError {
        if let Some(src_loc) = src_loc {
            Self::err(msg, label, src_loc.clone())
        } else {
            ParseError::AstMsgConvertError(msg.to_owned())
        }
    }
}
