...
use crate::ast::category::{Category, standard_categories};
...
use crate::ast::policycombinator::{
    PolicyCombinator, protected_policycombinators, standard_policycombinators,
};
use crate::ast::policyset::{PolicyEntry, PolicySet};
use crate::ast::rule::RuleDef;
use crate::ast::rulecombinator::{
    RuleCombinator, protected_rulecombinators, standard_rulecombinators,
};
...
use crate::ast::typedef::{TypeDef, standard_types};
    ...
    pub fn register(&self, elem: Rc<T>) -> Result<(), ParseError> {
        ...
        if let Some(n) = elem.fully_qualified_name() {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = m.entry(n.clone()) {
                ...
                // for duplicate symbols, we could show the location of both.
                // for now it is sufficient to show the second.

                // but we need to be able to extract a SrcLoc from the <T>.

                //Err(SrcError::err(
                //    "Duplicate {type_name} definition: '{n}'",
                //    &format!("second definiton of same name"),
                //    elem.span().clone()
                //))
...
impl Context {
    ...
    pub fn lookup_attribute_with_loc(
        &self,
        symbol: &str,
        source_ns: &[String],
        src_loc: Option<&SrcLoc>,
